...
commit_hw_sio [get_hw_sio_links -of_objects [get_hw_sio_linkgroups {LINKGROUP_0}]]
set_property TXPRE {1.94 dB (01000)} [get_hw_sio_links -of_objects [get_hw_sio_linkgroups {LINKGROUP_0}]]
commit_hw_sio [get_hw_sio_links -of_objects [get_hw_sio_linkgroups {LINKGROUP_0}]]
set_property TXPOST {0.45 dB (00010)} [get_hw_sio_links -of_objects [get_hw_sio_linkgroups {LINKGROUP_0}]]
commit_hw_sio [get_hw_sio_links -of_objects [get_hw_sio_linkgroups {LINKGROUP_0}]]
set_property TXDIFFSWING {530 mV (00101)} [get_hw_sio_links -of_objects [get_hw_sio_linkgroups {LINKGROUP_0}]]
commit_hw_sio [get_hw_sio_links -of_objects [get_hw_sio_linkgroups {LINKGROUP_0}]]
...
for {set i 40} { $i < $m} {incr i} {
  ...
  puts "$quad_name:$display_name: LOGIC.ERRBIT_COUNT.BEFOREREFRESH=[get_property {LOGIC.ERRBIT_COUNT} [lindex [get_hw_sio_links] [expr {$i - 40}]]]"
}
puts "===================="
puts "===================="
...
for {set i 40} { $i < $m} {incr i} {
  ...
  set quad_name [lindex [split [get_property {NAME} [lindex [get_hw_sio_gts] $i]] /] 6]
  puts "$quad_name:$display_name: LOGIC.ERRBIT_COUNT.BEFOREREFRESH=[get_property {LOGIC.ERRBIT_COUNT} [lindex [get_hw_sio_links] [expr {$i - 40}] ]]"
  set errbit_count_before($i) [get_property {LOGIC.ERRBIT_COUNT} [lindex [get_hw_sio_links] [expr {$i - 40}] ]]
...
for {set i 40} { $i < $m} {incr i} {
  ...
  puts "$quad_name:$display_name: DESCRIPTION=[get_property {DESCRIPTION} [lindex [get_hw_sio_links] [expr {$i - 40}] ]]"
  puts "$quad_name:$display_name: STATUS=[get_property {STATUS} [lindex [get_hw_sio_links] [expr {$i - 40}] ]]"
  puts "$quad_name:$display_name: QPLL0_STATUS=[get_property {QPLL0_STATUS} [lindex [get_hw_sio_plls] $qpll0]]"
  puts "$quad_name:$display_name: LINE_RATE=[get_property {LINE_RATE} [lindex [get_hw_sio_links] [expr {$i - 40}] ]]"
  puts "$quad_name:$display_name: LOGIC.ERRBIT_COUNT=[get_property {LOGIC.ERRBIT_COUNT} [lindex [get_hw_sio_links] [expr {$i - 40}] ]]"
  set errbit_count_after($i) [get_property {LOGIC.ERRBIT_COUNT} [lindex [get_hw_sio_links] [expr {$i - 40}] ]]
  ...
  set totalreceived [get_property {RX_RECEIVED_BIT_COUNT} [lindex [get_hw_sio_links] [expr {$i - 40}] ]]
...
#  }
  puts "$quad_name:$display_name: RX_RECEIVED_BIT_COUNT=[get_property {RX_RECEIVED_BIT_COUNT} [lindex [get_hw_sio_links] [expr {$i - 40}] ]]"
  puts "$quad_name:$display_name: TX_PATTERN=[get_property {TX_PATTERN} [lindex [get_hw_sio_links] [expr {$i - 40}] ]]"
  puts "$quad_name:$display_name: RX_PATTERN=[get_property {RX_PATTERN} [lindex [get_hw_sio_links] [expr {$i - 40}] ]]"
  puts "$quad_name:$display_name: LOOPBACK=[get_property {LOOPBACK} [lindex [get_hw_sio_links] [expr {$i - 40}] ]]"
  puts "$quad_name:$display_name: TX_ENDPOINT=[get_property {TX_ENDPOINT} [lindex [get_hw_sio_links] [expr {$i - 40}] ]]"
  puts "$quad_name:$display_name: RX_ENDPOINT=[get_property {RX_ENDPOINT} [lindex [get_hw_sio_links] [expr {$i - 40}] ]]"
  puts "$quad_name:$display_name: NAME=[get_property {NAME} [lindex [get_hw_sio_gts] [expr {$i - 40}] ]]"
