...
create_clock -period 5 -name clk_pin_p -waveform {0 2.5} [get_ports clk_pin_p]
set clk_rx [get_clocks -of [get_nets clk_rx] ]
set clk_tx [get_clocks -of [get_nets clk_tx] ]
set_input_delay  -clock clk_pin_p            1    [get_ports {rst_pin rxd_pin}]
set_input_delay  -clock clk_pin_p      -min  0.5    [get_ports {rst_pin rxd_pin}]
create_clock -period [get_property PERIOD $clk_tx] -name clk_tx_virtual
...
set_output_delay -clock clk_tx_virtual       1    [get_ports {txd_pin spi_clk_pin led_pins[*]} ]

# Path specific timing constraints
set_output_delay -clock clk_tx_virtual -clock_fall 0.200 [get_ports spi_clk_pin] -add_delay
...
set_max_delay 5 -from $clk_rx -to $clk_tx
